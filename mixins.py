...
class WinCombinationMixin:
    @staticmethod
    def __get_vertical_win_combination(horizont_combinations: list[list[int]]):
        for row_number in range(len(horizont_combinations)):
            yield [row[row_number] for row in horizont_combinations]

    @staticmethod
    def __get_diagonal_values(initial_list, values_list):
        for row in initial_list:
            index = next(values_list)
            yield row[index]

    def __get_diagonal_win_combination(self, horizont_combinations: list[list[int]], lenght):
